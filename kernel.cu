...
#define OFFSET (BLOCKS * BLOCK_SIZE)

__global__ void histogram_kernel(unsigned int* input, unsigned int* bins,
    unsigned int num_elements, unsigned int num_bins) {

	unsigned int tmp;
    int global = blockIdx.x * BLOCK_SIZE + threadIdx.x;
	int cycles = num_elements / (BLOCK_SIZE * BLOCKS) + 1;
	
	for (int i = 0; i < cycles; i++) {
		if (global + (OFFSET * i) < num_elements) {
			tmp = input[global + (OFFSET * i)];
			tmp = atomicAdd(&bins[tmp], 1u);
		}
	}
}
...
__global__ void convert_kernel(unsigned int *bins32, uint8_t *bins8,
	...
	bins8[g_id] = (bins32[g_id] > 255) ? 255 : bins32[g_id];
}
